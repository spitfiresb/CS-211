...
def decode(s: str) -> str:
    ...
    def r_decode(row: int, col: int, size: int):
        ...
        if command == BLACK:
            ...
            half_size = size // 2
            r_decode(row, col, half_size)  # NW
            r_decode(row, col + half_size, half_size)  # NE
            r_decode(row + half_size, col + half_size, half_size)  # SE
            r_decode(row + half_size, col, half_size)  # SW
...
X......X
""".strip()
#decode(linear_1) == expected
print(decode(linear_1))
